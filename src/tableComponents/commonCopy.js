...
// 页面从vuex转换数据字典
export function dicData(data, keyWrod, fn, $this, opt = 'options', isFilter = true) {
  let _this = $this
  let clone = fn
  let datas = data
  let arr = []
  let filters = []
  datas.forEach(function (item) {
    if (keyWrod === 'WarehousingCompany') {  // 仓储公司（数据字典后期改了）
      let o = {}
      ...
      filtersObj['value'] = `(${keyWrod} eq ${item.Id})`
      filters.push(filtersObj)
    } else if(keyWrod === 'Warehouse') {  // 实际存放库（数据字典后期改了）
      let o = {}
