...
// 页面从vuex转换数据字典
export function dicData2(dicName, keyWrod, fn, $this, opt = 'options', isFilter = true) {
    let _this = $this;
    let clone = fn;
    // let datas = dicName
    // let datas = _this.$store.state.dicBook.dic[dicName]
    _this.$store.dispatch('initFn', dicName);
    let datas = JSON.parse(localStorage.getItem(dicName)) || [];
    let arr = [];
    let filters = JSON.parse(localStorage.getItem(`${keyWrod}`)) || [];
    if (filters.length !== 0) {
        datas.forEach(function (item) {
        ...
        localStorage.setItem(`${keyWrod}_filter`, JSON.stringify(filters));
        let storeObj = {};
        storeObj[keyWrod] = arr;  // {BusinessType: arr}
        localStorage.setItem(keyWrod, JSON.stringify(arr));
    }
...
export function dicData(dicName, keyWrod, fn, fn2, fn3, $this, opt = 'options', isFilter = true) {
    let _this = $this;
    let clone = fn;
    let urlAppend = fn2;
    let o = fn3;
    // _this.$store.dispatch('initFn', dicName)
    let datas = null;
    // let datas = JSON.parse(localStorage.getItem(`${keyWrod}`))  // 和下面的有冲突 一个id 一个code
    let dataFilters = JSON.parse(localStorage.getItem(`${keyWrod}_filter`));

    function setData(isHas, datas, dataFilters) {
        let arr = [];
        let filters = [];
        if (isHas) {
            datas.forEach(function (item) {
                if (keyWrod === 'WarehousingCompany' || keyWrod === 'WarehoseingCompanyId' || keyWrod === 'WarehousingCompanyId') {  // 仓储公司（数据字典后期改了）
                    let o = {};
                    o['label'] = item.WarehousingCompany;
                    o['values'] = item.Id;
                    arr.push(o);
                    ...
                    let o = {};
                    o['label'] = item.Address;
                    o['values'] = item.Id;
                    arr.push(o);
                    let filtersObj = {};
                    filtersObj['label'] = item.Address;
                    filtersObj['value'] = `(${keyWrod} eq ${item.Id})`;
                    filters.push(filtersObj);
                } else {
                    let o = {};
    ...
    if (datas !== null) {
        setData(false, datas, dataFilters);
    } else {
        let url = localStorage.getItem('rbsSite') + '/';
        o(urlAppend(url + dicName, {r: Math.random()})).get().then(function (data) {
            datas = data.data;
            if (datas.length === 0) {
                return;
            }
            setData(true, datas, dataFilters);
        });
    }
}

// 页面从vuex转换数据字典
export function codeDicData(dicName, keyWrod, fn, fn2, fn3, $this, opt = 'options', isFilter = true) {
    let _this = $this;
    let clone = fn;
    let urlAppend = fn2;
    let o = fn3;
    // _this.$store.dispatch('initFn', dicName)
    let datas = null;
    // let datas = JSON.parse(localStorage.getItem(`${keyWrod}`))
    let dataFilters = JSON.parse(localStorage.getItem(`${keyWrod}_filter`));
